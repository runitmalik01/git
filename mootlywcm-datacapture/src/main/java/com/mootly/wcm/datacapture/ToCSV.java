...
public class ToCSV {
 ...
 private void rowToCSV(Row row) {
     ...
     if(row != null) {
         ...
         if (lastCellNum > 0 ) lastCellNum = 10;
         for(int i = 0; i <= lastCellNum; i++) {
        	 if (i > 8) {
        		 break;
        	 }
             cell = row.getCell(i);
             if (row.getRowNum() != 0 && ( row.getCell(1) == null || this.formatter.formatCellValue(row.getCell(1)).trim().equals(""))) break;
             if (i == 0 && row.getRowNum() != 0) {
            	 csvLine.add(this.formatter.formatCellValue(row.getCell(1)));
             }
             if(cell == null) {
                 csvLine.add("");
             }
                 ...
                 if(cell.getCellType() != Cell.CELL_TYPE_FORMULA) {
                	 //amit
                	 String retValue = this.formatter.formatCellValue(cell);
                	 if (retValue != null && retValue.equals("IFSC CODE")) {
                		 retValue = "IFSC_CODE";
                	 }
                	 else if (retValue != null && retValue.equals("MICR CODE")) {
                		 retValue = "MICR_CODE";
                	 }
                	 else if (retValue != null && retValue.equals("BRANCH NAME")) {
                		 retValue = "BRANCH_NAME";
                	 }    
                	 ...
                	 if (i == 0 && row.getRowNum() != 0) {
                		 csvLine.add(bankName);
                	 }
                	 else {
                		 csvLine.add(retValue) ;//this.formatter.formatCellValue(cell));
                	 }
                     //end amit
                 }
                 else {
     ...
     if ( csvLine != null && csvLine.size() > 0 && csvLine.get(0) != null && !csvLine.get(0).equals(",")) 
