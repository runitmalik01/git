...
public class TestITR1 {
	...
	public void testTaxRefundSingleSalary () {
		try {
			...
			formSixteenDetail.setPan_deductor("AAPPP1111A");
			...
			formSixteenDetail.setIncome_chargable_total(483470.85D);
			...
			deductionDocumentDetail.setInvestment(127287D);
			...
			tdsOthersDetail.setTan_Deductor("DELQ00425F");
			tdsOthersDetail.setTds_Certificate("123456");
			tdsOthersDetail.setFinancial_Year("2012");
			tdsOthersDetail.setTotal_TaxDeductor(2800D);
			tdsOthersDetail.setP_Amount(2800D);
			tdsFromothersDocument.setTotal_Amount(2800D);	
			...
			System.out.println("TaxLiability : Amount on which user has to pay tax ::"+TaxLiability);
			...
			System.out.println("TotalInterest:::"+intrstPay.getIntrstPayUs234A());
			...
			BigInteger RefundIncome = indianCurrencyHelper.bigIntegerRound(Double.parseDouble(returnMap.get("BalTaxPayable").toString()));
			BigInteger RefundFinalIncome = BalTaxPayable.multiply(new BigInteger("-1"));
			...
			System.out.println("RefundIncome(BalTaxPayable):::"+RefundIncome);
			assertTrue("Failed Refund Income", (RefundFinalIncome.equals(new BigInteger("70"))));
		} catch (Exception e) {
