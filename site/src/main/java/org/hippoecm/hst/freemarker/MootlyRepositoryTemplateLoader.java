...
public class MootlyRepositoryTemplateLoader implements TemplateLoader {
        ...
        try {
            ...
            if(session.itemExists(absPath)) {
                ...
                if(item.isNode()) {
                	...
                	if (theNode.getPrimaryNodeType() != null && theNode.getPrimaryNodeType().equals("mootlywcm:pdpagedocument")) {
                		//this is the MEAT to convert the ENTIRE page into a presentable freemarker SCRIPT
                		// as of now lets get all the blocks and then append them together
                		if (theNode.hasProperty("mootlywcm:blocks")) {
                			PropertyIterator propertyIterator = theNode.getProperties("mootlywcm:blocks");
                		}
                	}
