...
import in.gov.incometaxindiaefiling.y2012_2013.ITR3;
...
public class ITR3XmlGeneratorService  {

	private static Logger log = LoggerFactory.getLogger(ITR3XmlGeneratorService .class);
	 ...
	 * @throws Exception
	 */
	public static Map<String,Object> generateITR3(FinancialYear financialYear,Map<String,HippoBean> inputBeans) throws Exception {
		...
		itr3.setScheduleVIA(deductionVIASchedules.getScheduleVIA(itr, financialYear, inputBeans));
		...
		PartB_TI partB_TI = new PartB_TI(formSixteenDocument, salaryIncomeDocument, houseProperty, otherSourcesDocument,
				...
				incomeFromFirmsDocument);
		itr3.setPartBTI(partB_TI.getPartBTI(itr, financialYear, inputBeans));
		...
		itr3.setPartBTTI(partB_TTI.getPartBTTI(itr, financialYear, inputBeans));
		...
		PartA_Gen1 partA_Gen1 = new PartA_Gen1(formSixteenDocument, salaryIncomeDocument, houseProperty, otherSourcesDocument,
				...
				scheduleESRDocument, deductionSchedTenADocumemt,scheduleAMTCDocument, incomeFromFirmsDocument);
		itr3.setPartAGEN1(partA_Gen1.getPartAGEN1(itr, financialYear, inputBeans));

		CapitalGainDocumentSchedules capitalGainDocumentSchedules = new CapitalGainDocumentSchedules(capitalAssetDocument);
		itr3.setScheduleCGFor23(capitalGainDocumentSchedules.getSchedulecCgFor23(itr, financialYear, inputBeans));
		...
		itr3.setScheduleSI(iTRScheduleSI.getScheduleSI(itr, financialYear));

		ForeignIncomeScheduleFSI foreignIncomeScheduleFSI = new ForeignIncomeScheduleFSI(foreignIncomeDocument);
		itr3.setITRScheduleFSI(foreignIncomeScheduleFSI.getITRScheduleFSI(itr));

		TRDetailsSchedule tRDetailsSchedule = new TRDetailsSchedule(taxReliefDocument);
		itr3.setScheduleTR1(tRDetailsSchedule.getScheduleTR1(itr));
		...
		itr3.setScheduleFA(fADetailsSchedule.getScheduleFA(itr, financialYear, inputBeans));
		...
		ITR_ScheduleIF iTR_ScheduleIF = new ITR_ScheduleIF(firmsPartnerDocument, memberPersonalInformation);
		...
		itr3.setScheduleBPA(iTR3_ScheduleBPA.getScheduleBPA(itr));
	 ...
	 * @throws Exception
	 */
	public static String generateITR3(HstRequest request,HstResponse response) throws Exception {
		...
		Map<String,Object> outputMap = generateITR3(financialYear, inputBeans);
		String xml = null;
		if (outputMap != null) {
