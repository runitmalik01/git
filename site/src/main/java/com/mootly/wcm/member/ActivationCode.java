...
public class ActivationCode extends BaseComponent {
	...
	public void doAction(HstRequest request, HstResponse response)
		...
		Member member= MemberService.getActiveCode(request, email);
		log.warn("i am at member if");
		       if(member!= null)
		          {
			        log.warn("hello how are you?");
			
		       if((member.getEmail().toString()).equals(email))
			    {
		        EmailMessage ms = new EmailMessage();
		        ms.setTo(new String[] {email});
		        createEmailform(request,ms,member);
	...
	public EmailMessage createEmailform(HstRequest request,EmailMessage msg,Member member){
