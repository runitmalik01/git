...
public class TdsFromSalaryDocument extends BaseDocument implements ContentNodeBinder,FormMapFiller,CompoundChildUpdate {
	static final public String NAMESPACE = "mootlywcm:tdsfromsalarydocument";
	...
	private final static Logger log = LoggerFactory.getLogger(TdsFromSalaryDocument.class); 

	private List<TdsFromSalaryDetail> tdsfromSalaryDetailList;

	public final List<TdsFromSalaryDetail> getTdsSalaryDetailList() {
		if (tdsfromSalaryDetailList == null) tdsfromSalaryDetailList= getChildBeans(PROP_DETAIL_BEAN);
		return tdsfromSalaryDetailList;
	}

	public final void setSalaryIncomeDetailList(List<TdsFromSalaryDetail> tdssalaryDetailList) {
		this.tdsfromSalaryDetailList = tdssalaryDetailList;
	}
	
	public final void addTdsSalaryDetail(TdsFromSalaryDetail tdsSalaryetail) {
		getTdsSalaryDetailList();
		if (tdsfromSalaryDetailList == null) tdsfromSalaryDetailList = new ArrayList<TdsFromSalaryDetail>();
	...
	public boolean bind(Object content, javax.jcr.Node node)
		...
		try {
			TdsFromSalaryDocument tdssalary = (TdsFromSalaryDocument) content;
	...
	public <T extends HippoBean> void cloneBean(T sourceBean) {
		...
		TdsFromSalaryDocument salaryIncomeDocument = (TdsFromSalaryDocument) sourceBean;
	...
	public void update(HippoBean child) {
		...
		if (child.getCanonicalUUID() == null) {
			TdsFromSalaryDetail source =(TdsFromSalaryDetail) child;
			addTdsSalaryDetail(source);
		}
		boolean found = false;
		List<TdsFromSalaryDetail> listOfChildren = getTdsSalaryDetailList();
		for (HippoBean o:listOfChildren) {
			...
			if (child.getCanonicalUUID() != null && child.getCanonicalUUID().equals(o.getCanonicalUUID())) {
				...
				TdsFromSalaryDetail destination =(TdsFromSalaryDetail) o;
				TdsFromSalaryDetail source  = (TdsFromSalaryDetail) child;
