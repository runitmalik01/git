...
public class Comments extends BaseComponent {
	...
	private String createCommentNode(HstRequest request, WorkflowPersistenceManager wpm, HippoDocumentBean document) throws ObjectBeanPersistenceException {
		...
		final String commentFolderPathForReseller = commentFolderPathForReseller(request);

		if(StringUtils.isNotBlank(commentFolderPathForReseller)){
			log.warn("Site content base bean is not found");
			return wpm.createAndReturn(commentFolderPathForReseller, Constants.NT_COMMENT, commentNodeName, true);
		}else
	...
	// new Comments path for resellers
	private String commentFolderPathForReseller(HstRequest request){
		String newPath = null;
		HippoBean siteContentBase = getSiteContentBaseBeanForReseller(request);
		if(siteContentBase != null){
			newPath = siteContentBase.getPath()+"/cms/news";
			return newPath;
