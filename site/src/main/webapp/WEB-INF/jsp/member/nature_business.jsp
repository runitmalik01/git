					...
					<fieldset>
									...
									<select id="business_code" name="business_code" class="uprcase">
										...
										<c:forEach var="catBusinessCode" items="${catBusinessCode}">
											<optgroup label="${catBusinessCode.key}">
												<c:forEach var="subCatBussCode"
													items="${catBusinessCode.value}">
													<option
						...
						<div class="row show-grid">
							<div class="col-md-4">
								...
								<div class="rowlabel">
									<input id="tradeName_ProprietorshipSec" class="uprcase"
										maxlength="125" name="tradeName_ProprietorshipSec" type="text"
										value="${childBean.tradeName_ProprietorshipSec}" />
								</div>
							</div>
							...
							<div class="col-md-4">
								...
								<div class="rowlabel">
									<input id="tradeName_ProprietorshipLast" class="uprcase"
										maxlength="125" name="tradeName_ProprietorshipLast"
										type="text" value="${childBean.tradeName_ProprietorshipLast}" />
								</div>
							</div>
						</div>
					</fieldset>
				...
				<table class="table table-bordered">
					...
					<c:if test="${not empty parentBean}">
						<c:forEach items="${parentBean.natureBusinessDetailList}"
							var="businessDetail">
							<tr>
								<td class="uprcase"><c:forEach var="catBusinessCode"
										items="${catBusinessCode}">
										<c:forEach var="subCatBussCode"
											items="${catBusinessCode.value}">
											<c:if
												test="${businessDetail.business_Code == subCatBussCode.key}">
												<c:out value="${subCatBussCode.value}"></c:out>
											</c:if>
										</c:forEach>
									</c:forEach></td>
								<td><c:out
