...
<form id="searchSubmit" action="${doSearch}" method="get" onsubmit="sanitizeRequestParam(document.forms['searchSubmit']['query'], '${searchText}')">
  <table>
    <tr><td><input class="search-field gray" type="text" name="query" value="<fmt:message key="searchform.search" />"/>
    <input type="submit" value="${searchText}" class="search-button"/></td></tr>
  </table>
</form>
</div>