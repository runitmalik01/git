...
var valueQuestion2bBtn1;
var valueQuestion2bBtn2;
var valueQuestion2bText;
var onQuestion2bAction1=function(){};
var onQuestion2bAction2=function(){};

var valueQuestion3bBtn1;
var valueQuestion3bBtn2;
var valueQuestion3bBtn3;
var valueQuestion3bText;
var onQuestion3bAction1=function(){};
var onQuestion3bAction2=function(){};
var onQuestion3bAction3=function(){};
...
function showMessageBox(text,btn,cx,cy, path){
  var url=(path ? path : popupPathToCommon) + "messageBox.jsp?text=" + encodeURIComponent(text) +
...
//Reimplemented to exclude XSS vulnerabilities during TW43111 fix
//see comments in question2b.jsp for details
function show2ButtonModalDialog(text, btn1, btn2, action1, action2, def, cx, cy, path) {
	valueQuestion2bBtn1=btn1;
	valueQuestion2bBtn2=btn2;
	valueQuestion2bText=text;
    var url = (path ? path : "") + "question2b.jsp?action1=" + action1 + "&action2=" + action2 + "&default=" + def;
    if (!cx) cx = 360;
    if (!cy) cy = 180;
    return openModalDialog(url, "", cx, cy);
}
function show3ButtonModalDialog(text, btn1, btn2, btn3, action1, action2, action3, def, cx, cy, path) {
	valueQuestion3bBtn1=btn1;
	valueQuestion3bBtn2=btn2;
	valueQuestion3bBtn3=btn3;
	valueQuestion3bText=text;
    var url = (path ? path : "") + "question3b.jsp?action1=" + action1 + "&action2=" + action2 + "&action3=" + action3 +
              "&default=" + def;
    if (!cx) cx = 360;
    if (!cy) cy = 180;
    return openModalDialog(url, "", cx, cy, "");
...
function xml_unescape(strInput) {
    ...
    strInput = strInput.replace(/\&quot;/gi, '\"');
    strInput = strInput.replace(/\&apos;/gi, '\'');
    strInput = strInput.replace(/\&39;/gi, '\'');
    strInput = strInput.replace(/\&nbsp;/gi, ' ');
    strInput = strInput.replace(/\&amp;/gi, '&');
    return strInput;
}
...
function assignURL(url) {
    if (parent.openDocFromDocView) {
        ...
        window.location.assign(url); //change specific to VMware
    }
}
...
function checkObjectDeleted(objectType, id, message) {
    var intID = parseInt(id);
    if(intID > 0) {
    	if (objectType=='Group' || objectType=='Role') {
    		ManageGroupDwrServer.isGroupByIdExists(intID,{ callback:callbackMethod, async:false });
    	}
    }
    if(isReturnValue)
        return  true
    else {
        showMessageBox(message, 'OK', 360, 180);
        return false;
    }
}
function checkObjectAvailability(objectType, id, messageDeleted, messageNoAccess) {
    var intID = parseInt(id);
    if(intID > 0) {
    	if (objectType=='Group' || objectType=='Role') {
    		ManageGroupDwrServer.checkGroupAvailability(intID,{ callback:callbackMethod, async:false });
    	}
    }
    if(isReturnValue == 0)
        return  true
    	...
    	if (isReturnValue == 1) {
	        showMessageBox(messageDeleted, 'OK', 360, 180);
    	} else {
	        showMessageBox(messageNoAccess, 'OK', 360, 180);
    	}
        return false;
    }
}

function validateContentNotParsedSymbols(str){
    
	var strk = String.fromCharCode(
		0x1,  0x2,  0x3,  0x4,  0x5,  0x6,  0x7,
	        0x8,  0xB,  0xC,  0xE,  0xF,  0x10, 0x11,
	        0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
	        0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F
		);
			
    var regtext = '[' + strk +']';
    var rez = str.search(regtext);
    if(rez == -1 ) return "";
    else {
        if(rez>10) return str.substr(rez-10, 10);
        else return null;
    }
}
