...
function int234(){
	var aytaxmp=$('aytaxmp').value;
	$('aysfall').value=$('aytaxd').value*1-$('aytaxp').value*1;
	if($('aysfall').value<0){
		$('aysfall').value=0;
	}
	if($('ddate').value==7&&$('aytaxmp').value>7){
		$('dateA').value=aytaxmp*1-7*1;
	}else
		if($('ddate').value==9&&$('aytaxmp').value>9){
			$('dateA').value=aytaxmp*1-9*1;
		}else {
			$('dateA').value=0;
		}
	...
	if($('aytaxd').value>=10000){
		$('atiq3').value=Math.round($('atsfq3').value*3/100);
	}else{
		$('atiq3').value=0;
	}
