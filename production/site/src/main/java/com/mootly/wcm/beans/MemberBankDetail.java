...
/**
 * User: vivek
 * BD_BANK_NAME: Jun 29, 2010
 * Time: 11:26:35 AM
 */

// this bean is used for processapplication.jsp

@Node(jcrType = "mootlywcm:MemberBankDetail")
public class MemberBankDetail extends BaseDocument implements ContentNodeBinder, FormMapFiller {
	static final public String NAMESPACE = "mootlywcm:MemberBankDetail";            
	static final public String NODE_NAME = "MemberBankDetail";
	...
	public boolean bind(Object content, javax.jcr.Node node)
		...
		try {
			MemberBankDetail memberbankdetail = (MemberBankDetail) content;

			node.setProperty(BD_ACC_NUMBER,memberbankdetail.getBD_ACC_NUMBER());

			node.setProperty(BD_BANK_NAME,memberbankdetail.getBD_BANK_NAME());

			node.setProperty(BD_ECS,memberbankdetail.getBD_ECS());

			node.setProperty(BD_TYPE_ACC,memberbankdetail.getBD_TYPE_ACC());

			node.setProperty(BD_ADD_BANK_BRANCH,memberbankdetail.getBD_ADD_BANK_BRANCH());

			node.setProperty(BD_MICR_CODE,memberbankdetail.getBD_MICR_CODE());
