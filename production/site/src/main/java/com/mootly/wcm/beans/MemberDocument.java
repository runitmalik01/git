...
@Node(jcrType = "mootlywcm:memberdocument")
@TagAsTaxDataProvider(type=TaxDataProviderType.INCOME)
public class MemberDocument extends BaseDocument implements ContentNodeBinder,FormMapFiller,CompoundChildUpdate {
	static final public String NAMESPACE = "mootlywcm:memberdocument";
	static final public String NODE_NAME = "memberdocument";
	final String PROP_DETAIL_BEAN="mootlywcm:memberdetail";
	private String itFolderUuid;
	
	private final static Logger log = LoggerFactory.getLogger(MemberDocument.class); 

	private List<MemberDetail> salaryIncomeDetailList;

	public final List<MemberDetail> getSalaryIncomeDetailList() {
		if (salaryIncomeDetailList == null) salaryIncomeDetailList= getChildBeans(PROP_DETAIL_BEAN);
		return salaryIncomeDetailList;
	}

	public final void setSalaryIncomeDetailList(List<MemberDetail> salaryIncomeDetailList) {
		this.salaryIncomeDetailList = salaryIncomeDetailList;
	}
	
	public final void addSalaryIncomeDetail(MemberDetail salaryIncomeDetail) {
		getSalaryIncomeDetailList();
		if (salaryIncomeDetailList == null) salaryIncomeDetailList = new ArrayList<MemberDetail>();
	...
	public boolean bind(Object content, javax.jcr.Node node)
		...
		try {
			MemberDocument salaryincome = (MemberDocument) content;
        	...
        	if (salaryincome.getSalaryIncomeDetailList() != null && salaryincome.getSalaryIncomeDetailList().size() > 0 ){ 
        		for (MemberDetail salaryIncomeDetail:salaryincome.getSalaryIncomeDetailList()) {
	...
	public <T extends HippoBean> void cloneBean(T sourceBean) {
		...
		MemberDocument salaryIncomeDocument = (MemberDocument) sourceBean;
	...
	public void update(HippoBean child) {
		...
		if (child.getCanonicalUUID() == null) {
			MemberDetail source =(MemberDetail) child;
			addSalaryIncomeDetail(source);
		}
		boolean found = false;
		List<MemberDetail> listOfChildren = getSalaryIncomeDetailList();
		for (HippoBean o:listOfChildren) {
			...
			if (child.getCanonicalUUID() != null && child.getCanonicalUUID().equals(o.getCanonicalUUID())) {
				...
				MemberDetail destination =(MemberDetail) o;
				MemberDetail source  = (MemberDetail) child;
