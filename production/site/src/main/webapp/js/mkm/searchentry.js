            ...
            for(var i=0;i<clearedcontrols.length;i++){
		   		...
		   		if(ctrl){
					if(ctrl.options){
                        ...
                        if( clearedcontrols[i] == 'publicationstatus' || clearedcontrols[i] == 'to_rating' || clearedcontrols[i] == 'maxresults')
                            ctrl.selectedIndex=ctrl.options.length-1;
                        else {
                            if(ctrl.name == 'locale') {
                                for (var ix =0 ;ix < ctrl.options.length; ix++) {
	    						    var ctrlOption = ctrl.options[ix];
		    					    ctrlOption.selected = ctrlOption.value==defLocale? true : false;
						        }
						    } else ctrl.selectedIndex = 0;
                        }
		   		   	} else if(ctrl.value){
                        if (eval("formctrl.radionone"+ctrl.name)) {
                            if(ctrl.name == 'owner') setRadioNone(ctrl.name, 'none');
                            else {
                            ...
                            if(ctrl.name == 'author' || ctrl.name == 'creator' || ctrl.name == 'manager')
                                ctrl.value = '';
                            else if (ctrlDefaultValues[ctrl.name]) ctrl.value = ctrlDefaultValues[ctrl.name];
    		   		   		else ctrl.value = '';
		   		   		}
...
function validateForm(searchForm) {
	if (typeof(searchForm.id) != 'undefined' && searchForm.id == 'id_searchForm2' && searchForm.searchString.value == '') {
		alert('Article ID cannot be blank.');
		return false;
	}
	if (typeof(searchForm.id) != 'undefined' && searchForm.id == 'id_searchForm' && (searchForm.searchString.value == '' && searchForm.product.selectedIndex==0 && searchForm.category.selectedIndex==0) ) {
		alert('Please enter a search string or select a value from Product or Category');
		return false;
	}
   	var isValidated = true;
	//isValidated = checkDates( searchForm ); //custom
	
	//if( isValidated && window.checkRequired ) isValidated = checkRequired(searchForm); //custom
	//if( isValidated && window.addHiddenDatesFields ) window.addHiddenDatesFields( searchForm ); //custom
